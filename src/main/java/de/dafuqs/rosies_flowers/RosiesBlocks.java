...
public class RosiesBlocks {
	...
	public static List<SaplingSet> SAPLING_SETS = new ArrayList<>() {{
		add(new SaplingSet("dogwood", MapColor.WHITE_GRAY, false));
		add(new SaplingSet("red_dogwood", MapColor.DULL_PINK, false));
		add(new SaplingSet("blue_wisteria", MapColor.WATER_BLUE, false));
		add(new SaplingSet("purple_wisteria", MapColor.TERRACOTTA_PURPLE, false));
		add(new SaplingSet("yellow_poinciana", MapColor.YELLOW, true));
	}};
	...
	public static class SaplingSet extends BlockSet {
		...
		private final boolean hasBlossomingLeaves;

		public Block sapling;
		public Block pottedSapling;
		public Block leaves;
		public Block leafCarpet;
		public Block blossomingLeaves;
		public Block blossomingLeafCarpet;

		public SaplingSet(String name, MapColor mapColor, boolean hasBlossomingLeaves) {
			this.name = name;
			this.mapColor = mapColor;
			this.hasBlossomingLeaves = hasBlossomingLeaves;
		}
		
		public void register() {
			this.leaves = registerBlockWithBlockItem(name + "_leaves", new LeavesBlock(AbstractBlock.Settings.copy(Blocks.OAK_LEAVES).sounds(BlockSoundGroup.AZALEA_LEAVES).mapColor(mapColor)));
			this.leafCarpet = registerBlockWithBlockItem(name + "_carpet", new LeafCarpetBlock(AbstractBlock.Settings.copy(Blocks.WHITE_CARPET).sounds(BlockSoundGroup.AZALEA_LEAVES).mapColor(mapColor).nonOpaque()));
			
			if(this.hasBlossomingLeaves) {
				this.blossomingLeaves = registerBlockWithBlockItem("blossoming_" + name + "_leaves", new LeavesBlock(AbstractBlock.Settings.copy(Blocks.OAK_LEAVES).sounds(BlockSoundGroup.AZALEA_LEAVES).mapColor(mapColor)));
				this.blossomingLeafCarpet = registerBlockWithBlockItem("blossoming_" + name + "_carpet", new LeafCarpetBlock(AbstractBlock.Settings.copy(Blocks.WHITE_CARPET).sounds(BlockSoundGroup.AZALEA_LEAVES).mapColor(mapColor).nonOpaque()));
			}
			
			SaplingGenerator saplingGenerator = new SaplingGenerator("rosies_" + name, Optional.empty(), Optional.of(RegistryKey.of(RegistryKeys.CONFIGURED_FEATURE, RosiesFlowers.id(name + "_tree"))), Optional.empty());
			...
			if(this.hasBlossomingLeaves) {
				registry.add(blossomingLeaves, 30, 60);
			}
		}
		...
		public void registerClient() {
			...
			if(this.hasBlossomingLeaves) {
				BlockRenderLayerMap.INSTANCE.putBlocks(RenderLayer.getCutout(), blossomingLeafCarpet);
			}
		}
		...
		public void addEntries(ItemGroup.Entries entries) {
			...
			if(this.hasBlossomingLeaves) {
				entries.add(blossomingLeaves);
				entries.add(blossomingLeafCarpet);
			}
		}
