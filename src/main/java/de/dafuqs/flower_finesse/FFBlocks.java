...
import de.dafuqs.flower_finesse.blocks.*;
...
public class FFBlocks {
	...
	public static class SaplingSet extends BlockSet {
		...
		public void register() {
			...
			SaplingGenerator saplingGenerator = new SaplingGenerator("rosies_" + name, Optional.empty(), Optional.of(RegistryKey.of(RegistryKeys.CONFIGURED_FEATURE, FlowerFinesse.id(name + "_tree"))), Optional.empty());
	...
	public static class WoodSet extends BlockSet {
		...
		public static final BlockSetType BLOCK_SET_TYPE = BlockSetTypeBuilder.copyOf(BlockSetType.OAK).register(FlowerFinesse.id("wood"));
		...
		public WoodSet(String name, MapColor mapColor, MapColor topLogMapColor) {
			...
			this.woodType = new WoodTypeBuilder().register(FlowerFinesse.id(name), BLOCK_SET_TYPE);
		}
		...
		public void register() {
			...
			Block signBlock = new TerraformSignBlock(FlowerFinesse.id("entity/signs/dogwood"), AbstractBlock.Settings.create().mapColor(mapColor).solid().instrument(NoteBlockInstrument.BASS).noCollision().strength(1.0F).burnable());
			Block wallSignBlock = new TerraformWallSignBlock(FlowerFinesse.id("entity/signs/dogwood"), AbstractBlock.Settings.create().mapColor(mapColor).solid().instrument(NoteBlockInstrument.BASS).noCollision().strength(1.0F).dropsLike(signBlock).burnable());
	...
	static Block registerBlockWithBlockItem(String name, Block block) {
		Block b = Registry.register(Registries.BLOCK, FlowerFinesse.id(name), block);
		Registry.register(Registries.ITEM, FlowerFinesse.id(name), new BlockItem(block, new Item.Settings()));
		return b;
	}

	static Block registerBlockWithItem(String name, Block block, Item item) {
		Block b = Registry.register(Registries.BLOCK, FlowerFinesse.id(name), block);
		Registry.register(Registries.ITEM, FlowerFinesse.id(name), item);
		return b;
	}

	static Block registerBlock(String name, Block block) {
		return Registry.register(Registries.BLOCK, FlowerFinesse.id(name), block);
	}
